...
RG %<>% mutate(lcat5=lencat(Length, w=5)) %<>% mutate(lcat10=lencat(Length, w=10)) %<>% filter(!is.na(RG$Age))  %<>% mutate(logTL=log10(Length), logOR=log10(OR), Site=factor(Site)) 
...
(Age_plot<-ggplot(RG, aes(x=Age, y=..count.., fill=Site)) + geom_bar(binwidth=1, color="black", position=position_dodge(0.9, preserve = "single")) + facet_grid(cols = vars(RG$Sex)) + labs(x = "Age",  y = "Count") + scale_fill_grey() + theme_classic () + xlim(0,5)+ theme(plot.margin = margin(t=0.1, l=0.1, r=0.1, b=0.1, unit = "cm"), text = element_text(size = 15), legend.position = "bottom"))

(MAge_plot<-ggplot(Male, aes(x=Age, y=..count.., fill=Site)) + geom_bar(color="black", position=position_dodge(0.9, preserve = "single")) + labs(x = "Age",  y = "Count", title = "Male") + ylim(0,80) + scale_fill_grey() + theme_classic () + xlim(0,5) + theme(plot.margin = margin(t=0.1, l=0.1, r=0.1, b=0.1, unit = "cm"), text = element_text(size = 15), legend.position = "bottom"))

(FAge_plot<-ggplot(Female, aes(x=Age, y=..count.., fill=Site)) + geom_bar(color="black", position=position_dodge(0.9, preserve = "single")) + labs(x = "Age",  y = "Count", title = "Female") + ylim(0,80) + scale_fill_grey() + theme_classic () + xlim(0,5) + theme(plot.margin = margin(t=0.1, l=0.1, r=0.1, b=0.1, unit = "cm"), text = element_text(size = 15), legend.position = "bottom"))

(Ageplot <- ggarrange(MAge_plot, FAge_plot,
...
ggplot(RG, aes(x=logOR, y=logTL)) + geom_point(aes(color=Site)) + facet_grid (rows=vars(Sex)) + theme_classic() + scale_color_manual(values=c("black", "gray")) + labs(x= "Log(Otolith Radius) (mm)", y="Log (Total Length) (mm)")+ theme(legend.position="bottom")+ geom_smooth(method = "lm", aes(group = Site, color=Site, linetype=Site))


(MaleOR_plot<-ggplot(Male, aes(x=logOR, y=logTL)) + geom_point(aes(color=Site)) + theme_classic() + scale_color_manual(values=c("black", "gray")) + labs(x= "Log(Otolith Radius) (mm)", y="Log (Total Length) (mm)", title = "Male")+ theme(legend.position="bottom")+ geom_smooth(method = "lm", aes(group = Site, color=Site, linetype=Site)) + theme(plot.margin = margin(t=0.1, l=0.1, r=0.1, b=0.1, unit = "cm"), text = element_text(size = 15), legend.position = "bottom") + ylim(1,3)+ xlim(0,0.6))


(FemaleOR_plot<-ggplot(Female, aes(x=logOR, y=logTL)) + geom_point(aes(color=Site)) + theme_classic() + scale_color_manual(values=c("black", "gray")) + labs(x= "Log(Otolith Radius) (mm)", y="Log (Total Length) (mm)", title = "Female")+ theme(legend.position="bottom")+ geom_smooth(method = "lm", aes(group = Site, color=Site, linetype=Site)) + theme(plot.margin = margin(t=0.1, l=0.1, r=0.1, b=0.1, unit = "cm"), text = element_text(size = 15), legend.position = "bottom") + ylim(1,3) + xlim(0,0.6))

(ORplot <- ggarrange(MaleOR_plot, FemaleOR_plot,
                    nrow = 2, common.legend = TRUE, legend="bottom"))
