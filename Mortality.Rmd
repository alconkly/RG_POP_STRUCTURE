...
lmCC_site<- lm(lnct~Age*Site, data=RGCounts_site_2)
summary(lmCC_site)
Anova(lmCC_site)
...
(Site_Mortality_Table<- tbl_regression(lmCCs_site) %>%  
...
lmCC_sex<- lm(lnct~Age*Sex, data=RGCounts_sex_2)
summary(lmCC_sex)
Anova(lmCC_sex)
...
(Mortality_plot<-ggplot(data=RGCounts, aes(x=Age, y=log(n))) + geom_point() + ylim(0,7) + labs(x= "Age", y="log(Catch [n])") + stat_smooth(method = lm, se=FALSE, data = RGCounts_3, colour="black") + theme_classic() + theme(plot.margin = margin(t=0.1, l=0.1, r=0.1, b=0.1, unit = "cm"), text = element_text(size = 15)))
